...
OBJECTS = $(patsubst src/%.c, build/%.o, $(wildcard src/*.c) $(wildcard src/**/*.c))
OBJECTS += $(patsubst src/%.cpp, build/%.o, $(wildcard src/*.cpp) $(wildcard src/**/*.cpp))
DEPS = $(OBJECTS:.o=.d)

ifeq ($(OS), Windows_NT)
	MKDIR_BUILD = if not exist build md build
	MKDIR_BUILD_GB = if not exist build/gb md build/gb
	RMDIR = rd /s /q
else
	MKDIR_BUILD = mkdir -p build
	MKDIR_BUILD_GB = mkdir -p build/gb
	RMDIR = rm -rf
endif
